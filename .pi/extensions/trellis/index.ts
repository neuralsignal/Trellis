...
function thinkingIntent(text: string) {
  ...
  if (/error|failed|failure|panic|exception/.test(s))
    return "Analyzing failure cause";
  if (/test|verify|check|typecheck|lint/.test(s))
    return "Planning verification steps";
  if (/plan|approach|design|strategy/.test(s))
    return "Structuring the implementation approach";
  if (/implement|change|edit|modify|refactor/.test(s))
    return "Reasoning through code changes";
  if (/inspect|search|locate|read|context/.test(s))
