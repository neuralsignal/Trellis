...
- **Platform files**: `.claude/`, `.cursor/`, `.opencode/`, `.codex/`, `.pi/`, and the shared `.agents/skills/` layer.
...
| --- | --- | --- |
| settings/config | `.claude/settings.json`, `.cursor/hooks.json`, `.codex/hooks.json`, `.pi/settings.json` | Register hooks, plugins, extensions, or platform behavior. |
...
| agents | `.claude/agents/`, `.cursor/agents/`, `.codex/agents/`, `.pi/agents/` | Define `trellis-research`, `trellis-implement`, and `trellis-check`. |
| skills | `.claude/skills/`, `.cursor/skills/`, `.opencode/skills/`, `.agents/skills/` | Capability descriptions that auto-trigger or can be read on demand. |
| commands/prompts/workflows | `.claude/commands/`, `.cursor/commands/`, `.opencode/commands/`, `.pi/prompts/` | Entry points explicitly invoked by the user. |

## Two Platform Integration Modes
