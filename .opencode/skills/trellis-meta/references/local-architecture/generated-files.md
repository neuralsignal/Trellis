...
| settings | `.claude/settings.json`, `.cursor/hooks.json`, `.codex/hooks.json`, `.pi/settings.json` | Tell the platform when to run hooks or plugins. |
| agents | `.claude/agents/`, `.cursor/agents/`, `.codex/agents/`, `.pi/agents/` | Define agents such as `trellis-research`, `trellis-implement`, and `trellis-check`. |
| skills | `.claude/skills/`, `.cursor/skills/`, `.opencode/skills/`, `.agents/skills/` | Skills that auto-trigger or can be read by AI. |
| commands/prompts/workflows | `.claude/commands/`, `.cursor/commands/`, `.opencode/commands/`, `.pi/prompts/` | Explicit user-invoked command or workflow entry points. |
